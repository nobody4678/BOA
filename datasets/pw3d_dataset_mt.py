...
class PW3D(Dataset):
    ...
    def __getitem__(self, index):
        ...
        kp2d_i, img_i, pose_i, betas_i, smpl_j2ds_i = self.process_sample(img.copy(), pose, betas, keypoints, smpl_j2ds, center, scale, flip, pn, rot, sc, is_train=False)
        ...
        # bbox
        _h = 200 * scale
        bbox_i = np.stack([center[0], center[1], _h])
        item['wholeimg'] = img
        item['ori_bbox'] = bbox_i
        item['ori_imgshape'] = np.stack([ori_width,ori_height]).astype(np.float32)
        ...
        for _ in range(self.num_aug):
            ...
            kp2d_i, img_i, pose_i, betas_i, smpl_j2ds_i = self.process_sample(img.copy(), pose, betas, keypoints, smpl_j2ds, center, scale, flip, pn, rot, sc, is_train=True)
